...
export async function getStandingsByTournamentId(
  tournamentId: number,
): Promise<StandingRow[]> {
...
export async function ensureStandingsForTournament(
  tournamentId: number,
): Promise<void> {
  ...
  await prisma.$transaction(async (tx: TransactionClient) => {
    ...
    const existing = await gs.findMany({
      where: { tournamentId },
      select: { playerId: true },
    });
    const existingSet = new Set(
      existing.map((e: { playerId: number }) => e.playerId),
    );

    const toCreate = players.filter(
      (p: { id: number }) => !existingSet.has(p.id),
    );
    ...
    if (standings.some((s: any) => s.rank == null)) {
      ...
      for (const [_, list] of byGroup) {
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (row.rank == null) {
            updates.push(
              gs.update({ where: { id: row.id }, data: { rank: i + 1 } }),
            );
          }
        }
      }
