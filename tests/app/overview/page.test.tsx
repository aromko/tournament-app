...
describe("OverviewPage (/overview)", () => {
  ...
  it("renders tournaments in a data table with name, status and actions", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /Actions for/i }),
    ).toHaveLength(2);
  ...
  it("opens a Register dialog to add a player name", async () => {
    h.getTournaments.mockResolvedValueOnce([{ id: 1, name: "Spring Open" }]);
    ...
    const actionsBtn = screen.getByRole("button", {
      name: /Actions for Spring Open/i,
    });
    await userEvent.click(actionsBtn);

    // Click Register
    const registerItem = await screen.findByRole("menuitem", {
      name: /Register/i,
    });
  ...
  it('shows only "View" action for started tournaments', async () => {
    ...
    // Open actions menu
    const actionsBtn = screen.getByRole("button", {
      name: /Actions for Championship Finals/i,
    });
    await userEvent.click(actionsBtn);
