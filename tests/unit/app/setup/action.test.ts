...
describe("setup actions", () => {
  ...
  describe("createTournamentPlayers", () => {
    it("returns message when tournamentId missing or invalid", async () => {
      ...
      res = (await createTournamentPlayers(
        { tournamentId: "abc" },
        new FormData(),
      )) as any;
      expect(res.message).toMatch(/invalid tournament id/i);
    ...
    it("upserts players and deletes extras, then redirects to teams page", async () => {
      ...
      expect(h.tx.player.update).toHaveBeenCalledWith({
        where: { id: 2 },
        data: { name: "Bobby" },
      });
      expect(h.tx.player.createMany).toHaveBeenCalledWith({
        data: [{ name: "Cara", tournamentId: 9 }],
      });
  ...
  describe("deleteTournamentPlayer", () => {
    it("throws error when ids are invalid", async () => {
      ...
      fd.set("playerId", "5");
      await expect(deleteTournamentPlayer(fd)).rejects.toThrow(
        /invalid tournament or player id/i,
      );

      const fd2 = new FormData();
      fd2.set("tournamentId", "7");
      fd2.set("playerId", "oops");
      await expect(deleteTournamentPlayer(fd2)).rejects.toThrow(
        /invalid tournament or player id/i,
      );
    });
    ...
    it("deletes the player and redirects back to teams page", async () => {
      ...
      expect(h.prisma.player.deleteMany).toHaveBeenCalledWith({
        where: { id: 5, tournamentId: 7 },
      });
  ...
  describe("assignTournamentTeams", () => {
    it("returns message when tournamentId missing or invalid", async () => {
      ...
      res = (await assignTournamentTeams(
        { tournamentId: "NaN" },
        new FormData(),
      )) as any;
      expect(res.message).toMatch(/invalid tournament id/i);
    ...
    it("applies removals and assignments, updates tournament, and redirects to root", async () => {
      ...
      expect(h.tx.player.updateMany).toHaveBeenCalledWith({
        where: { id: { in: [10] }, tournamentId: 12 },
        data: { groupNumber: null },
      });
      // Unassigned players (not in assignments) should be cleared as well
      expect(h.tx.player.updateMany).toHaveBeenCalledWith({
        where: { tournamentId: 12, id: { notIn: [1, 2, 3] } },
        data: { groupNumber: null },
      });
      // Assigned players should be updated to their group numbers
      expect(h.tx.player.updateMany).toHaveBeenCalledWith({
        where: { id: 1, tournamentId: 12 },
        data: { groupNumber: 1 },
      });
      expect(h.tx.player.updateMany).toHaveBeenCalledWith({
        where: { id: 2, tournamentId: 12 },
        data: { groupNumber: 2 },
      });
      expect(h.tx.player.updateMany).toHaveBeenCalledWith({
        where: { id: 3, tournamentId: 12 },
        data: { groupNumber: 3 },
      });
      expect(h.tx.tournament.update).toHaveBeenCalledWith({
        where: { id: 12 },
        data: { numberOfGroups: 3, started: true },
      });
